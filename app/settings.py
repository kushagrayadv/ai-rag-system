...
class Settings(BaseSettings):
    ...
    def load_settings(cls) -> "Settings":
        """
        Tries to load the settings from the ClearML configuration. If the configuration does not exist, it initializes the settings from the .env file and default values.
        ...
        if task:
            logger.info("Loading settings from ClearML configuration.")
            config_dict = task.get_configuration_object()
            if config_dict:
                return Settings(**config_dict)
        
        logger.warning("Failed to load settings from ClearML. Defaulting to loading the settings from the '.env' file.")
        return Settings()

    def export(self) -> None:
        """
        Exports the settings to the ClearML configuration.
        """
        task = Task.current_task()
        if task:
            ...
            task.set_configuration_object(name="settings", config_dict=config_dict)
        else:
            logger.warning("No active ClearML task. Settings were not exported.")
