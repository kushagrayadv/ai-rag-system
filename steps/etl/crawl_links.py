...
def crawl_links(user: UserDocument, links: List[str]) -> List[str]:
  ...
  for link in tqdm(links):
    ...
    if not successful_crawl:
      failed_crawled_links.append(crawled_domain)

    metadata = _add_to_metadata(metadata, crawled_domain, successful_crawl)

  metadata["failed_crawled_links"] = failed_crawled_links

  task = Task.current_task()
  task.upload_artifact("crawled_links_metadata", metadata)
